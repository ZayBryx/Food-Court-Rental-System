<?php
    ...
    if(empty($first_name) || empty($last_name) || empty($username) || empty($password) || empty($confirm_password)){
        header('Location: /signup_user?errorMsg=Fill all fields');
        exit();
    }

    // Validation: Check if passwords match
    if ($password !== $confirm_password) {
        header("Location: /signup_user.php?errorMsg=Passwords do not match");
        exit();
    }

    // Validation: Check if email is valid
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: /signup_user.php?errorMsg=Invalid email format");
        exit();
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        header("Location: /signup_user.php?errorMsg=Username already exists");
        exit();
    }
    ...
    if (mysqli_query($connection, $insert_user_query)) {
        header("Location: /index.php?successMsg=User registered successfully");
        exit();
    } else {
        header("Location: /signup_user.php?errorMsg=Failed to register user");
        exit();
    }
?>
